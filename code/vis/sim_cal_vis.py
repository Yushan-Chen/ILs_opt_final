...
IL_SMILES_PATH = '/home/ychen3338/project_2/data/after_m_tox.csv'
OUTPUT_PATH = 'simles_vis.csv'
N_JOBS = 24
...
for smi, vis in zip(train_smiles, train_tox):
    if vis >= np.log(10):
        continue
    mol = Chem.MolFromSmiles(smi)
    if mol:
...
il_filtered = il_df[il_df['IL_smile'].isin(filtered_df['IL_smile'])][['IL_smile', 'm_pre', 'tox_pre']].drop_duplicates(subset='IL_smile')
