...
params = {'fp_radius': 1, 'fp_length': 1214}  
...
P = np.full((X_pred.shape[0], 1), 1, dtype='float32')

X_pred = np.concatenate([X_pred, T, P], axis=1)

df['co2_pre'] = best_model.predict(X_pred)

df_filtered = df[df['co2_pre'] >= 0.2]
output_cols = ['IL_smile', 'Cation', 'Anion', 'm_pre', 'tox_pre','vis_pre','co2_pre']
df_filtered[output_cols].to_csv('/home/ychen3338/project_2/data/after_m_tox_vis_co2.csv', index=False)